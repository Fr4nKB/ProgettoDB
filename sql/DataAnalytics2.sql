...
BEGIN
	WITH
	CTE AS (SELECT M.ModelloId, Seq.SequenzaId, T*(L.Quantita + SUM(IFNULL(Numero,0))) AS TempoTotale, SUM(DISTINCT Op.Stipendio) AS TotRisorse, SUM(IFNULL(Numero,0))/COUNT(LP.CodiceLotto) AS TotUnitaPerse, 
	...
	CTE2 AS (SELECT ModelloId, SequenzaId, TempoTotale, TotRisorse/(Quantita+TotUnitaPerse) AS CostoMedioUnita, NumRotazioni/NumOperaiImpiegati AS RotazioniXOperaio, 
				(LAG(TotUnitaPerse, 1) OVER(PARTITION BY ModelloId, SequenzaId ORDER BY DataProduzione) / TotUnitaPerse * 100) - 100 AS AndamentoUnitaPerse, TotUnitaPerse
			FROM CTE)
